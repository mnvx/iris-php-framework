        ...
        <?php foreach(Config::$url_prefix_format as $format_name => $format_value) { ?>
          <?php if ($format_value['display'] && count($format_value['supported']) > 1) { ?>
            ...
            <fieldset data-role="controlgroup">
              ...
              <input type="radio" class="select-prefix" name="<?php echo $format_name; ?>" id="<?php echo $format_name.'_'.$supported_name; ?>" value="<?php echo $router->_url($format_name, $supported_name, true); ?>" <?php if ($supported_name == $router->get_url_prefix($format_name)) { ?> checked="checked" <?php } ?>/>
