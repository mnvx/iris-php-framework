<?php namespace IrisPHPFramework; ?>
	...
	<form action='<?php echo $router->url('profile_edit'); ?>' method='post' class='register-form'>
		<p>
			...
			<input type='text' name='name' class='text' id='name' value='<?php echo $view->escape($view->get('user_name')); ?>' />
		</p>
		<p>
			<label for='login'><?php echo _('E-mail'); ?>:</label>
			<input type='text' name='login' class='text' id='login' value='<?php echo $view->escape($view->get('user_login')); ?>' />
		</p>
		<p>
